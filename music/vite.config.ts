...
function copyWallAppPublicPlugin(): Plugin {
  let productionMapboxToken = ''

  return {
    name: 'copy-wall-app-public',
    configResolved(config) {
      productionMapboxToken = loadEnv(config.mode, config.envDir, '').VITE_MAPBOX_TOKEN?.trim() || ''
    },
    closeBundle() {
      ...
      if (!productionMapboxToken) {
        throw new Error(
          '[wall-runtime] VITE_MAPBOX_TOKEN is required to build the production Wall runtime',
        )
      }
      ...
      // The source bridge remains empty and safe to commit. The release bundle
      // receives the production token from Vite's resolved environment without
      // copying an ignored developer file or exposing the value in build logs.
      fs.writeFileSync(
        path.join(wallDest, 'mapbox-env.js'),
        `// Generated by the MUSIC production build. Do not commit this file.\n`
          + `(function () {\n`
          + `  var token = ${JSON.stringify(productionMapboxToken)};\n`
