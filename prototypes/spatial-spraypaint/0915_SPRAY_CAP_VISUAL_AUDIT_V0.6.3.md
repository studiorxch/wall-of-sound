...
- **Dwell behavior:** a stationary dwell strengthens all three bands (mist/ring/core) through ordinary compositing of repeated zero-length round-capped strokes — re-verified; nothing is gated by dwell.
...
- **Current visual strengths:** live-verified this pass across the required matrix at default size — a stationary dot reads as a clean hot core → ring → fading mist bullseye, center visibly painted, not hollow; short/long straight lines render as one smooth continuous tube with no scalloping; a sharp zigzag stays fully connected through every corner; a closed loop closes cleanly; a reversal/backtrack reads as one solid connected stroke; a slow line reads fuller than a fast line, both continuous; a 0°/45° angle comparison shows coherent whole-plume flare; three overlapping Fill sweeps show visibly more outer buildup than one sweep while the core stays soft-edged.
- **Known visual defects:** all plume field magnitudes were carried forward unchanged from the prior pass's judgment calls, not re-measured; core physics (`coreDensity`/`coreOpacity`/etc.) remain screenshot-judgment baseline from the original commit. Distance's outer-grows-faster-than-inner response was proven via 4 dedicated unit tests rather than live mouse simulation, since `sprayDistance` is driven by hand-tracking depth in this build, not a desktop mouse control.
...
- **Pink Dot plume physical calibration** — tune the core's own base values AND the seven `plume*` fields (ring/mist radius-thickness-opacity, distance gain, flare strength) against a real loaded-dot reference at multiple distances/angles/speeds once available (cap #2's exact next test).
