...
- **Classification:** NEEDS CALIBRATION (corrected this pass; explicitly physically unverified — see Preserved Distinctions)
- **Physical vs. digital:** represents the real "Pink Dot" loaded-cap category. Core+halo mechanism from commit `31fc282`, now corrected to be distance-sensitive, oblique-flared, dab-spaced, and center+ring shaped against physical reference footage (a real Pink Dot Fat Cap demo video) — magnitudes are still a judgment call, not a measurement.
- **Documented width/range:** 34–54 (unmeasured).
- **Digital Wall-space width:** `baseRadius` 42 (unchanged).
- **Dot profile:** `coreDensity` 1.46, `coreOpacity` 0.34 (both highest of the fat family, unchanged) plus `haloRadius` 2.4× resolved radius, `haloOpacity` 0.05 per draw (unchanged base values) — now modulated by four new correction fields rather than applied as a flat constant.
- **Halo correction (new this pass):** `haloDistanceGain` 0.85 — the live resolved Size (relative to this cap's own `baseRadius`) scales halo strength: small size → suppressed toward a clean hot dot/line ("near-wall"); large size → amplified toward a pronounced bloom ("pulled back"). `haloFlareAnisotropy` 0.55 — the halo elongates into an ellipse along travel direction above a ~0.12 units/ms velocity threshold (an oblique-spray flare); a true dwell stays a perfect circle. `haloDabSpacing` 0.85 — halo draws are gated by travel distance while moving, so a swept stroke deposits discrete overlapping dabs (a visibly scalloped, textured edge) instead of one smooth airbrushed bar; a true dwell is never gated. `haloRingBias` 0.6 — the gradient gains a soft moat then a brighter outer band, so a large halo reads as a genuine center-plus-ring, not a single glow; the core disc stays fully opaque underneath (never hollow, unlike Ring/Donut).
- **Moving stroke profile:** `endpointBehavior` "punchy", `accumulationRate` 1.38 (highest of the fat family, unchanged) — builds density fastest.
- **Core/body character:** loaded center, strong dot personality — structurally distinct from New York Fat (separate halo mechanism) and now distance/motion-responsive rather than a flat constant.
- **Edge character:** `edgeFalloff` 0.76 (softer pass-to-pass expansion) plus the halo's own soft gradient boundary, now moat-shaped rather than a single linear fade.
- **Overspray character:** wide/pronounced (`particleCount` 26, `particleSpread` 1.2, `splatterProbability` 0.14) — untouched by this correction, halo-only in scope.
- **Motion/speed response:** lower than New York Fat (`velocityResponse` 0.42) — stays denser at speed, consistent with a "loaded" character. Unrelated to the new halo-flare velocity threshold, which only governs the halo's ellipse shape.
- **Orientation behavior:** symmetric round (the CORE stays symmetric; only the halo can now elongate on movement).
- **Dwell behavior:** halo redrawn at the same point strengthens the center and widens the visible soft field, purely via ordinary compositing — unchanged and re-verified (dwell dot materially larger/denser than a quick tap; the new dab-spacing gate never applies to a zero-distance dwell).
- **Drip status:** `dripTendency` 0.72 — highest of the fat family.
- **Fill default:** ON (`defaultFillMode: true`) — same fat-cap workflow default as New York/Astro/German Fat; Fill OFF remains a per-brush manual toggle for dense outline work.
- **Current visual strengths:** live-verified this pass — native size shows a distinct hot core plus a separated outer ring with a visible moat; small size produces a clean near-halo-free dot; large size produces a dramatically bigger, clearly separated center+ring bloom; a continuous sweep shows a visibly scalloped/dabbed edge instead of a smooth bar.
- **Known visual defects:** all four correction magnitudes (`haloDistanceGain`/`haloFlareAnisotropy`/`haloDabSpacing`/`haloRingBias`) were set by judgment against the reference footage, not measured; `haloRadius`/`haloOpacity`'s own base values are still unchanged from the original screenshot-judgment pass.
- **Target behavior:** near-wall → clean hot dot/line; pulled back → center + clear outer ring; oblique/moving → elliptical flare, not a circular blur; overlapping strokes → dusty layered fill, not a flat bar — all four now have a real mechanism; magnitude unverified against measurement.
- **Exact next calibration test:** side-by-side comparison against the reference footage's own near/far/oblique/overlap moments (ideally in the Calibration Bench once it supports video reference import — see its own "Known limitations" note) to tune the four correction magnitudes, plus a stationary-dot photo-matched pass to tune `haloRadius`/`haloOpacity` themselves.
...
- **Pink Dot** still has core+halo behavior, now corrected to be distance-sensitive/oblique-flared/dab-spaced/center-plus-ring (cap #2 above) against physical reference footage — remains physically unverified in exact magnitude. Core/overspray physics (`coreDensity`/`coreOpacity`/`particleCount`/etc.) are untouched — this was a halo-only correction.
...
7. ~~**Pink Dot Fat correction**~~ — **DONE.** See cap #2 above — halo now distance-sensitive (`haloDistanceGain`), oblique-flared (`haloFlareAnisotropy`), dab-spaced (`haloDabSpacing`), and center+ring shaped (`haloRingBias`) against physical reference footage. Core/overspray physics untouched; every other cap regression-tested unaffected. Magnitude of the four new fields is still a judgment call against the reference footage, not a measurement — see P1 below.
...
- **Pink Dot halo physical calibration** — tune `haloRadius`/`haloOpacity` (the base halo magnitudes) AND the four new correction fields (`haloDistanceGain`/`haloFlareAnisotropy`/`haloDabSpacing`/`haloRingBias`) against a real loaded-dot reference at multiple distances/speeds once available (cap #2's exact next test).
