...
- `src/SprayBrushEngine.ts` — `resolveShapedStampGeometry`/`shapedStampWidthAlongTravel`/`drawShapedStamp` (Oval Calligraphy / Rectangular Transversal), `resolveRingProfile`/`drawRingStamp` (Ring/Donut), `resolveStreakGate`/`renderStreakCore` (Dry/Streak), and `resolvePinkDotPlume`/`renderPinkDotPlume`/`resolveMouseSprayInput` (Pink Dot Fat's unified plume — see its own section below).
...
- **Classification:** NEEDS CALIBRATION (unified this pass; explicitly physically unverified — see Preserved Distinctions)
- **Physical vs. digital:** represents the real "Pink Dot" loaded-cap category. The stationary dot was corrected against physical reference footage in the prior pass; live testing then showed a **moving** stroke still read as two independent systems (a generic core stroke plus a separately-timed halo layered under it), not one coherent physical spray. This pass replaces the mechanism entirely: core, ring, and mist now resolve from ONE state and draw as one dab-spaced stamp.
- **Documented width/range:** 34–54 (unmeasured).
- **Digital Wall-space width:** `baseRadius` 42 (unchanged).
- **Deposition mechanism:** `depositionShape: "plume"` — exclusive to this cap. `SprayBrushEngine.resolvePinkDotPlume` resolves `coreRadius`/`coreOpacity`/`coreDensity`/`ringRadius`/`ringThickness`/`ringOpacity`/`mistRadius`/`mistOpacity`/`anisotropy`/`depositSpacing` together from the cap's own fields plus a canonical, input-neutral `SprayInputState` (`sprayAngle`/`sprayDistance`/`sprayOutput` — mouse V1 maps into this via `resolveMouseSprayInput`; a future Pencil input would add its own mapper without touching the resolver). `renderPinkDotPlume` draws mist, then ring, then core from that ONE state, inside one shared rotate/scale ellipse transform, at the SAME dab-gated points — the "ring" read is two stacked circles (opaque core drawn last, on top), not a moat-gradient trick.
- **Dot profile:** `coreDensity` 1.46, `coreOpacity` 0.34 (both highest of the fat family, unchanged — core physics untouched by this pass) plus `plumeRingRadius` 1.7× / `plumeRingThickness` 0.55 / `plumeRingOpacity` 0.24, and `plumeMistRadius` 2.6× / `plumeMistOpacity` 0.07 for the two atmospheric layers.
- **Distance response:** `plumeDistanceGain` 0.85 (same magnitude as the prior pass's `haloDistanceGain`, reused semantics) — the live resolved Size relative to `baseRadius` amplifies ring+mist beyond their own proportional scaling: near-wall (small Size) → suppressed toward a clean hot dot/line; pulled back (large Size) → pronounced center+ring+mist bloom. The core itself needs no extra gain — it already scales with resolved Size directly.
- **Angle + velocity flare, unified:** `plumeFlareStrength` 0.55 elongates the WHOLE plume (not halo-only) via `flareFactor = max(angleFactor, velocityFactor)` — an explicit simulated spray angle (new: Brush Studio's "Spray Angle" property, 0-45°, or the live Alt+scroll-wheel shortcut) and point velocity each independently drive elongation, the stronger one wins (no exaggerated stacking), and angle alone can flare a perfectly **stationary** dwell — live-verified: a stationary dot at 30-45° simulated angle renders a genuinely elliptical plume.
- **Deposit spacing, unified:** `plumeDabSpacing` 0.8 gates the WHOLE stamp (mist+ring+core together) by travel distance while a segment is actually moving; a true dwell (zero travel) is never gated, so dwell strengthening is unchanged. Spacing also widens at far distance, reinforcing dusty/gapped buildup rather than a uniformly bigger continuous bar.
- **Moving stroke profile:** `endpointBehavior` "punchy", `accumulationRate` 1.38 (highest of the fat family, unchanged) — builds density fastest; the same endpoint taper now applies to the whole plume stamp (mist/ring/core together), not just the core.
- **Core/body character:** loaded center, strong dot personality — structurally distinct from New York Fat, and now genuinely coherent on a moving stroke (center, ring, and mist visibly move/taper together, not as two separately-timed systems).
...
- **Overspray character:** wide/pronounced (`particleCount` 26, `particleSpread` 1.2, `splatterProbability` 0.14) — untouched by this pass, entirely separate from the plume mechanism.
- **Motion/speed response:** lower than New York Fat (`velocityResponse` 0.42) — stays denser at speed, consistent with a "loaded" character. Distinct from the plume's own velocity-driven flare contribution (elongation shape), which is a separate axis from density.
- **Orientation behavior:** symmetric at rest; the whole plume (not halo-only) elongates together under angle or velocity, sharing one transform.
- **Dwell behavior:** the plume stamp redrawn at the same point strengthens the whole plume (not just a halo layer) through ordinary compositing — re-verified; the dab-spacing gate never applies to a zero-distance dwell.
- **Drip status:** `dripTendency` 0.72 — highest of the fat family.
- **Fill default:** ON (`defaultFillMode: true`) — same fat-cap workflow default as New York/Astro/German Fat; Fill OFF remains a per-brush manual toggle for dense outline work. Fill's ceiling mechanism applies to the plume's core layer exactly as it did to the old core loop — architecture unchanged.
- **Current visual strengths:** live-verified this pass across the full close/far/curve/angle/speed/Fill matrix — near-wall reads as a clean tight dot/line; pulled back produces a dramatically larger, clearly separated center+ring+mist bloom; a **stationary** dwell at 30-45° simulated angle renders a genuinely elliptical plume (the brief's core visual-acceptance bar); a fast oblique sweep reads visibly lighter/thinner than the same angle swept slowly; a continuous sweep shows a visibly scalloped, dusty-layered edge, not a flat glowing bar; three overlapping Fill passes read as mottled atmospheric buildup, not a solid bar.
- **Known visual defects:** all plume field magnitudes were set by judgment against reference footage and the prior pass's proven values, not measured; core physics (`coreDensity`/`coreOpacity`/etc.) remain screenshot-judgment baseline from the original commit.
- **Target behavior:** near-wall → clean hot dot/line; pulled back → center + clear outer ring + mist; oblique/moving → elliptical flare (works for both stationary deposits and moving strokes) — not a circular blur; overlapping strokes → dusty layered fill, not a flat bar. All four now share one coherent mechanism; magnitude unverified against measurement.
- **Exact next calibration test:** side-by-side comparison against the reference footage's own near/far/oblique/overlap moments (ideally in the Calibration Bench once it supports video reference import — see its own "Known limitations" note) to tune the plume field magnitudes, plus a stationary-dot photo-matched pass to tune the core's own base values.
...
- **Pink Dot** now uses a dedicated unified plume mechanism (`depositionShape: "plume"`, cap #2 above) — core, ring, and mist resolve from one state and draw as one dab-spaced stamp, replacing the prior pass's separate core-stroke-plus-halo architecture. Distance-sensitive/oblique-flared/dab-spaced/center-plus-ring behavior all still apply, now unified across the whole moving plume rather than halo-only. Remains physically unverified in exact magnitude. Core/overspray physics (`coreDensity`/`coreOpacity`/`particleCount`/etc.) are untouched by this pass.
...
8. ~~**Pink Dot Fat unified plume**~~ — **DONE.** See cap #2 above — a moving stroke read as two independent systems (generic core stroke + separately-timed halo) even after item 7's correction; replaced with `depositionShape: "plume"`, a dedicated mechanism resolving core+ring+mist together from one state (`resolvePinkDotPlume`) and drawing them as one dab-spaced stamp. Adds an input-neutral simulated spray-angle control (Brush Studio "Spray Angle" property + live Alt+scroll-wheel shortcut) so flare can be tested on desktop; angle and velocity both drive elongation, the stronger one wins. Live-verified: stationary dwell flares purely from angle, fast/slow oblique sweeps taper naturally, overlapping Fill passes read as dusty buildup not a flat bar. Core/overspray physics untouched; every other cap regression-tested unaffected. Generic halo mechanism retired to 0 everywhere (preserved as dormant infrastructure, see P2). Magnitude of all plume fields is still a judgment call, not a measurement — see P1 below.
...
- **Pink Dot plume physical calibration** — tune the core's own base values AND the eight `plume*` fields (ring/mist radius-thickness-opacity, distance gain, flare strength, dab spacing) against a real loaded-dot reference at multiple distances/angles/speeds once available (cap #2's exact next test).
