...
- **Classification:** NEEDS CALIBRATION (dual-plume rework, then an outer-field/dwell refinement pass; explicitly physically unverified — see Preserved Distinctions)
- **Physical vs. digital:** represents the real "Pink Dot" loaded-cap category. The stationary dot was already reading well after the prior "unified plume" pass, but live testing of a **moving** stroke regressed: bullseye identity weakened, lines read lumpy/scalloped, joints/endpoints could visibly disconnect — root cause was the core itself being stamped as discrete dab-gated circles rather than drawn as a continuous stroke. The dual-plume pass fixed that with two coordinated CONTINUOUS layers of one cap. A follow-up refinement pass then fixed two remaining artifacts without touching that architecture: the outer layer read as a second smooth translucent tube (not an aerosol), and any near-zero-distance point deposited a full-size bulb regardless of real dwell time.
- **Documented width/range:** 34–54 (unmeasured).
- **Digital Wall-space width:** `baseRadius` 42 (unchanged).
- **Deposition mechanism:** `depositionShape: "plume"` — exclusive to this cap. `SprayBrushEngine.resolvePinkDotDualPlume` resolves ONE state, `{ inner, outer }`, from the cap's own fields plus the same canonical, input-neutral `SprayInputState` (`sprayAngle`/`sprayDistance`/`sprayOutput` — mouse V1 maps into this via `resolveMouseSprayInput`, unchanged). `inner` (`radius`/`opacity`/`density`/`falloff`/`anisotropy`) is drawn by `renderPinkDotInnerCore` as a normal continuous multi-pass stroked line, never gated by travel distance. `outer` (`ringRadius`/`ringThickness`/`ringOpacity`/`mistRadius`/`mistOpacity`/`anisotropy`) is drawn by `renderPinkDotOuterField` — a continuous aerosol DENSITY FIELD, not a second tube: `resolvePinkDotOuterFieldShells` builds several (5) concentric shells with a radially-biased opacity (a low mist baseline plus a Gaussian bump peaking at the ring radius), and `resolveOuterFieldTexture` modulates each shell's alpha along the travel direction with a deterministic sine-based grain (never reaching zero, so the envelope stays continuous while its density visibly varies) — all still drawn under the core every segment, never gated. The `plumeDabSpacing` field and its dab-gating state remain deleted — continuity still doesn't depend on a spacing threshold. The bullseye read still falls out of stacked opaque-over-translucent layers (core drawn last, on top), same as before, not a moat-gradient trick.
...
- **Dwell behavior:** genuinely dwell-driven (refinement pass). `resolvePinkDotDwellScale`, fed by engine-tracked real elapsed stationary time (`pinkDotDwellMs`), scales both layers' size together for a near-zero-distance point: a floor of ~0.22 at 0ms (a bare click, or the instant real movement stops) ramps linearly to full size by 900ms. A genuine hold still strengthens further through ordinary repeated-draw compositing on top of that ramp, same mechanism as before; a real moving segment always renders at full scale, untouched.
...
- **Current visual strengths:** live-verified across the dual-plume pass's required matrix at default size (stationary bullseye, continuous lines through corners/loops/reversals, coherent whole-plume flare, dusty Fill buildup — see the prior pass's notes), plus the refinement pass's own matrix: click-drag-immediately and drag-release-immediately both show no oversized bulb at either end, with the outer field already visibly grainy/textured rather than a flat tube; a 0.2s/0.5s/1s dwell comparison shows clean progressive growth from a small dot to a full hot-core/ring/mist bullseye; a long line, a curve, and a reversal all stay smooth and continuous with no scalloping or disconnection; three overlapping Fill sweeps read visibly denser and more atmospherically built-up than one sweep.
- **Known visual defects:** all plume field magnitudes were carried forward unchanged from the prior pass's judgment calls, not re-measured; core physics (`coreDensity`/`coreOpacity`/etc.) remain screenshot-judgment baseline from the original commit. Distance's outer-grows-faster-than-inner response was proven via 4 dedicated unit tests rather than live mouse simulation, since `sprayDistance` is driven by hand-tracking depth in this build, not a desktop mouse control. The outer field's shell count (5) and dwell-ramp duration (900ms) were both set by judgment/live-read, not measured against reference footage.
- **Target behavior:** near-wall → clean hot dot/line; pulled back → center + clear outer ring + mist, outer expanding faster than inner; oblique/moving → elliptical flare coherent across inner+outer, inner more restrained; overlapping strokes → dusty layered fill from the outer atmosphere, not a flat bar; moving line stays continuous through corners, reversals, and loops with no scalloping or disconnected joints. All achieved this pass; magnitude still unverified against physical measurement.
...
10. ~~**Pink Dot Fat dual-plume refinement — outer field + dwell-driven endpoints**~~ — **DONE.** See cap #2 above — item 9's outer layer still read as a second smooth translucent tube (two flat, uniform-opacity bands), and any near-zero-distance point (a bare click, an immediate release) deposited a full-size bulb regardless of real dwell time. Replaced `renderPinkDotOuterBand`'s two flat bands with `renderPinkDotOuterField`, sweeping 5 concentric density shells (`resolvePinkDotOuterFieldShells` — a low mist baseline plus a Gaussian bump at the ring radius) with a deterministic sine-based mist grain (`resolveOuterFieldTexture`, same technique family as Dry/Streak's `resolveStreakGate`) that never reaches zero, so the envelope stays continuous while density visibly varies — a genuine aerosol field, not literal repeated donuts. Added real dwell-time tracking (`pinkDotDwellMs`, `resolvePinkDotDwellScale`): a near-zero-distance segment (<15% of resolved radius) ramps from a ~0.22 floor to full size over 900ms of genuine stationary time, resetting instantly on real movement — applies to both layers together, never to a real moving segment. Dual-plume architecture, cap sizes, and manufacturer anchors untouched. Live-verified full required matrix: click-drag-immediately and drag-release-immediately both stay small at their ends; 0.2s/0.5s/1s dwell shows clean progressive bullseye growth; long line/curve/reversal stay smooth and continuous; three Fill sweeps read visibly denser than one. 434/434 tests passing (15 new).
...
- **Pink Dot plume physical calibration** — tune the core's own base values, the seven `plume*` fields (ring/mist radius-thickness-opacity, distance gain, flare strength), AND the refinement pass's own outer-field shell count / dwell-ramp duration against a real loaded-dot reference at multiple distances/angles/speeds/dwell-lengths once available (cap #2's exact next test).
