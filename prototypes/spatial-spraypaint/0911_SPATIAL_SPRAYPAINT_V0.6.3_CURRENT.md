...
## Pink Dot Unified Plume V1 — Core, Ring, And Mist From One Resolved State

Builds directly on the pass above. Live testing of that build showed the *stationary* dot was substantially improved, but a *moving* stroke still read as two independent systems: a generic fat-cap core stroke, with a separately-timed halo layered underneath it. This pass replaces that architecture for Pink Dot Fat specifically — nothing else changes.

**Root cause.** The old halo was a bolt-on: the core deposited via the SAME shared, continuous-line loop every other cap uses (no dab gating at all — a smooth stroked line), while the halo/ring deposited via its own separately-dab-gated mechanism. A smooth continuous core tube next to discretely-dabbed ring blobs is structurally "two systems," regardless of how good either looked in isolation.

**Fix — a new `depositionShape: "plume"`, exclusive to Pink Dot Fat.** A single pure resolver, `resolvePinkDotPlume(cap, input, velocity, dynamics)`, returns one `PinkDotPlumeState` — `coreRadius`, `coreOpacity`, `coreDensity`, `ringRadius`, `ringThickness`, `ringOpacity`, `mistRadius`, `mistOpacity`, `anisotropy`, `depositSpacing` — and a single `renderPinkDotPlume` method draws mist, then ring, then core from that ONE state, at the SAME dab-gated points, inside ONE shared rotate/scale ellipse transform. The "ring" read falls out of two stacked opaque-under-transparent circles (core drawn last, on top) — no moat-gradient trick needed anymore. Distance/angle/velocity all modulate the same resolved numbers every layer reads, so a moving stroke's center, ring, and mist now visibly move and taper together.

**Input-neutral architecture (per the brief).** `resolvePinkDotPlume` never touches mouse/pointer specifics — it consumes a canonical `SprayInputState` (`sprayAngle`, `sprayDistance`, `sprayOutput`). `resolveMouseSprayInput` is the ONLY place mouse-specific values (`point.width`, the Spray Angle control) get read for spray-physics purposes. A future Pencil input path would add its own `resolve*SprayInput` mapper (tilt → sprayAngle, pressure → sprayOutput) without touching the plume resolver at all — not built in this pass, by design.

**Distance response** reuses 82bf82c's proven close/small → clean, far/large → pronounced-bloom relationship for the ring/mist (`plumeDistanceGain`, same magnitude as the old `haloDistanceGain`); the core needs no extra gain since it already scales with the live resolved Size directly, same as every cap.

**Angle + velocity flare, unified.** `plumeFlareStrength` elongates the WHOLE plume (not halo-only) via `flareFactor = max(angleFactor, velocityFactor)` — an explicit simulated spray angle and point velocity each independently drive elongation, the stronger one wins rather than stacking into an exaggerated combined effect, and angle alone can flare a perfectly *stationary* dwell (velocity 0) — a genuine capability gap the old halo-only mechanism never had.

**Desktop angle control V1.** A new generic per-brush property, "Spray Angle" (0-45°, `PLUME_MAX_ANGLE_DEGREES`), alongside Size/Coverage/Fill in Brush Studio's General group and in the same per-brush override store — available on every cap, only visibly effective on a `"plume"` cap. A live painting shortcut, Alt+scroll-wheel over the wall, adjusts the same underlying override in ±3° steps without opening Brush Studio; a compact status readout ("Spray angle (Alt + scroll)") appears next to the Fill toggle only once the angle has actually moved off 0° (Creative Interface Doctrine: normal state stays quiet). Both paths write the identical per-brush `sprayAngle` override, recorded into stroke history for correct undo/redo replay.

**Deposit spacing, unified.** `plumeDabSpacing` now gates the WHOLE stamp (mist+ring+core together) by travel distance while a segment is actually moving; a true dwell (zero travel distance) is never gated, preserving continuous dwell strengthening exactly as before. `depositSpacing` also widens at far distance, reinforcing dusty/gapped buildup rather than just a bigger continuous bar.

**The old halo mechanism (`haloRadius`/`haloOpacity`/`haloDistanceGain`/`haloFlareAnisotropy`/`haloDabSpacing`/`haloRingBias`) is retired from Pink Dot Fat** — every value is now 0, same as every other cap — but preserved as generic, still fully tested, dormant infrastructure (the Visual Audit's own P2 "Loaded Dot/Halo as a general reusable trait" item), not deleted.

**Live-verified**, full matrix (close/far dots, close/far lines, a straight-on curve, a stationary dwell at 30°/45° simulated angle, fast vs. slow oblique sweeps, three overlapping Fill passes): near-wall reads as a clean tight dot/line; pulling back (Size) produces a dramatically larger, clearly separated center+ring+mist bloom; a genuinely elliptical plume appears on a **stationary** dwell purely from the simulated angle (the core visual-acceptance bar); a fast oblique sweep reads lighter/thinner than the same angle swept slowly (velocity's natural taper, un-exaggerated); a continuous sweep shows a visibly scalloped, dusty-layered edge, not a flat glowing bar; three overlapping Fill passes read as atmospheric, mottled buildup, not a solid bar. Brush Studio's Spray Angle slider live-updates the same real-engine preview. New York Fat (and every other cap) verified unaffected — no plume rows shown, no behavior change. No console errors.

**Files:** `SprayCapPresets.ts` (`"plume"` deposition shape, 8 new `plume*` fields, Pink Dot's old halo fields retired to 0), `SprayBrushEngine.ts` (`SprayInputState`, `resolveMouseSprayInput`, `PinkDotPlumeState`, `resolvePinkDotPlume`, `renderPinkDotPlume`, `plumeTravelSinceLastDab` state, `PLUME_MAX_ANGLE_DEGREES`), `BrushProperties.ts` (`sprayAngle` override + General row; 8 new read-only PAINT rows shown only for a `"plume"` cap), `BrushStudio.ts` (Spray Angle slider range wiring), `BrushPreview.ts` (`sprayAngle` threaded into the live preview), `DrawingToolRenderer.ts` (`ToolStrokeStyle.sprayAngle`), `main.ts` (`currentToolStyle` includes `sprayAngle`; Alt+wheel shortcut; compact status readout), `index.html` (status readout markup/CSS), `CalibrationBench.ts` (plume rows added to the property readout, a small necessary consequence of Pink Dot's migration off the halo mechanism — Calibration Bench mechanics themselves untouched), `SprayCapPresets.test.ts` / `SprayBrushEngine.test.ts` / `BrushProperties.test.ts` / `CalibrationBench.test.ts` (Pink-Dot-specific halo tests replaced with plume-equivalent coverage; a small "generic halo mechanism (dormant, preserved)" block added using a synthetic fixture so the retained mechanism stays regression-tested).

## Pink Dot Dual-Plume V2 — One Cap, Two Coordinated Continuous Layers (Inner Core + Outer Atmosphere)

Builds directly on the Unified Plume V1 pass above. Live testing of that build showed the *stationary* dot had substantially improved, but a *moving* stroke still regressed: bullseye identity weakened, lines read lumpy/scalloped, joints/endpoints could visibly disconnect. This pass replaces the single dab-gated whole-plume-stamp architecture with a genuinely two-layer continuous model — still ONE cap, ONE resolver, no new tool.

**Root cause.** V1's core, ring, and mist were all stamped together as discrete circles at dab-gated travel intervals — the same gating mechanism originally built for the halo/ring/mist layer had been extended to the CORE itself. That is what made even the line body (not just its atmosphere) look disconnected/lumpy on a moving stroke: gaps between successive stamps, not just between successive glows.

**Fix — `resolvePinkDotDualPlume(cap, input, velocity, dynamics)`.** One pure resolver returns `{ inner, outer }` from the same canonical `SprayInputState`/velocity/dynamics inputs V1 used — not two independent tools, two layers of one physical cap. `inner` (`radius`, `opacity`, `density`, `falloff`, `anisotropy`) is a normal continuous line core: drawn with `ctx.stroke()` on every segment, never gated by travel distance, exactly like every other cap's core already works. `outer` (`ringRadius`, `ringThickness`, `ringOpacity`, `mistRadius`, `mistOpacity`, `anisotropy`) is a continuous atmosphere sheath — mist band, then ring band, both stroked every segment under the core, also never gated. The `plumeDabSpacing` field and its `plumeTravelSinceLastDab` engine state are deleted entirely; continuity no longer depends on a spacing threshold at all.

**Bullseye, without a moat trick.** A stationary dwell draws a zero-length `moveTo→lineTo` stroke with `lineCap: "round"` at each of mist/ring/core — a standard canvas quirk renders that as a filled circle. Stacking mist (widest, faintest) under ring (medium) under core (narrowest, most opaque, drawn last/on top) reads as a genuine hot-center-plus-ring bullseye through simple opaque-over-translucent layering, not a gradient/moat profile. The core's opacity is never zero — Pink Dot stays deliberately non-hollow, structurally distinct from Ring/Donut's genuinely hollow annular core.

**Distance — outer grows faster than inner, structurally.** `plumeDistanceGain` applies only to the outer ring/mist; the inner core's radius is the plain resolved Size with no extra gain, so "outer expands faster than inner" is guaranteed by construction rather than tuned by feel.

**Flare — coherent across both layers, inner more restrained.** The existing `flareFactor = max(angleFactor, velocityFactor)` now drives both layers' anisotropy from one shared value: outer gets the full `plumeFlareStrength`, inner gets half of it (`PLUME_INNER_FLARE_RATIO = 0.5`) — the line body stays more circular/controlled while the atmosphere flares more freely, matching the brief's "moderate inner, stronger outer" split. The ellipse-via-transform technique (translate to segment midpoint, rotate to travel angle, scale the transversal axis, draw a local line) is reused unchanged from V1; skipped entirely at `anisotropy === 1`.

**Texture — via existing overspray, not a new mechanism.** Rather than building a second sparse-sampling system for the outer envelope, Pink Dot's overspray call now reads the dual-plume's own `outer.anisotropy` and bypasses `resolveOverspraySquashAngle`'s fixed-transversal-axis branch (built for Calligraphy-style fixed-orientation caps), so overspray's flare correctly follows travel direction and stays coherent with the outer envelope it sits on top of — texture comes from the codebase's pre-existing continuous particle mechanism, never from gaps in the envelope itself.

**Fill-ceiling stays core-only, unchanged from every other cap's convention.** `FILL_MODE_CORE_CEILING` and `fillLocalSaturation` apply to the inner core exactly as before; the outer atmosphere is never ceiling-limited. This is what produces the desired dusty-buildup character for free: the core stays visually capped across repeated Fill passes while the outer atmosphere genuinely and visibly accumulates, unbounded.

**Live-verified** in the current-host browser, full required matrix at default Pink Dot size: a stationary dot reads as a clean hot core → ring → fading mist bullseye, center visibly painted, not hollow; short and long straight lines render as one smooth continuous tube with soft atmosphere tapering at the endpoint, no scalloping; a sharp zigzag stays fully connected through every corner; a closed loop closes cleanly (its small unpainted center is the expected interior of a ring-shaped stroke, not a disconnection); a reversal/backtrack reads as one solid connected stroke; a slow line reads fuller and a fast line reads visibly lighter/thinner, both still continuous; a 0°-vs-45° spray-angle comparison shows the flared line clearly and uniformly narrower across its whole length, confirming angle affects the whole plume coherently; three overlapping Fill sweeps show visibly more built-up outer atmosphere than one sweep, while the core stays soft-edged rather than a flat bucket fill. Distance's outer-grows-faster-than-inner response is proven by four dedicated unit tests rather than live mouse simulation, since `sprayDistance` in this build is driven by hand-tracking depth, not a desktop mouse control (consistent with this pass's own instruction to leave desktop input untouched). No console errors from the rendering path (unrelated MediaPipe hand-tracking model-fetch failures, pre-existing and unaffected by this change, were the only console errors observed).

**Files:** `SprayCapPresets.ts` (`plumeDabSpacing` removed from all 16 presets and the interface; plume field docs rewritten for the dual-layer model), `SprayBrushEngine.ts` (`PinkDotInnerState`, `PinkDotOuterState`, `PinkDotDualPlumeState`, `resolvePinkDotDualPlume` replacing `resolvePinkDotPlume`; `renderPinkDotDualPlume`, `renderPinkDotOuterBand`, `renderPinkDotInnerCore` replacing `renderPinkDotPlume`; `plumeTravelSinceLastDab` state removed; overspray call updated to share the outer layer's anisotropy and bypass the fixed-transversal squash branch for Pink Dot), `BrushProperties.ts` (`plumeDabSpacing` row removed; two remaining plume rows relabeled to note outer-only/inner-half scope), `CalibrationBench.ts` (`plumeDabSpacing` readout row removed), `CalibrationBench.test.ts` / `SprayCapPresets.test.ts` / `SprayBrushEngine.test.ts` / `BrushPreview.test.ts` / `BrushProperties.test.ts` (updated/replaced coverage for the dual-plume model; gradient-based assertions rewritten to stroke-based ones, since Pink Dot no longer produces `createRadialGradient` calls at all). 419/419 tests passing, `tsc --noEmit` clean, production build clean.

## Pink Dot Dual-Plume Refinement — Outer Field + Dwell-Driven Endpoints

Live human testing of the V2 dual-plume build (above) surfaced two remaining artifacts, both explicitly scoped as deposition-quality refinements — the dual-plume architecture itself (one resolver, two coordinated continuous layers) was not touched: (1) the outer layer rendered as a smooth translucent tube around the inner line — a second hose, not an aerosol; (2) starts/ends could bulge into oversized circular bulbs even when no actual dwell occurred, because any near-zero-distance segment (a bare click, an immediate release) deposited the outer field at full native size regardless of real elapsed time.

**Outer field, fixed — a density field, not a second tube.** `renderPinkDotOuterBand`'s two flat, uniform-opacity capsule strokes (mist, then ring) are replaced by `renderPinkDotOuterField`, which sweeps several (5) thin concentric "shells" along the same segment every call, still with no distance-based gating. Each shell's base opacity comes from `resolvePinkDotOuterFieldShells` — a pure function combining a low, broad mist baseline (strongest near the core-side transition, fading toward the mist edge) with a Gaussian bump centered on the ring radius — giving "lower-density transition → stronger outer band → fading mist" without a moat-gradient trick or literal repeated donuts (the annular read falls out of the STACK of shells, not any single one). Each shell's alpha is further modulated along the travel direction by `resolveOuterFieldTexture`, a deterministic sine-based "grain" reusing the same position-derived technique as Dry/Streak's `resolveStreakGate` (no `Math.random`, replays identically) with a floor that never reaches zero — the spatial envelope stays continuous along the path; only the density inside it visibly varies, giving a genuinely dusty/mottled read instead of a flat glow.

**Endpoints, fixed — genuinely dwell-driven.** A new engine-tracked `pinkDotDwellMs` accumulates real elapsed stationary time: any segment traveling less than 15% of the resolved radius (`PLUME_DWELL_DISTANCE_RATIO`) counts as "dwelling in place" and adds its real timestamp delta; any segment with real movement resets the counter to 0 instantly. `resolvePinkDotDwellScale(dwellMs)` — pure, unit-testable — maps that elapsed time to a size multiplier: a floor of ~0.22 at 0ms (a bare click, or the instant after real movement stops), ramping linearly to 1 (full size) by 900ms (`PLUME_DWELL_FULL_MS`). This `dwellScale` multiplies BOTH layers' radii together for a dwell-type point only — a real moving segment always renders at scale 1, completely unaffected, so line-body continuity from the prior pass is untouched. Distance-based, not velocity-based: the actual failure mode (a near-zero-length stroke's round line-cap degenerating into a full-diameter circle) is a distance phenomenon, and this also correctly handles mid-stroke pauses (not just the stroke's literal first/last point) and an immediate release right after real movement (small real elapsed time since the last sample → near-floor scale), with no special-casing beyond the one distance gate.

**Live-verified** in the current-host browser, the full required matrix: click-drag-immediately and drag-release-immediately both show no oversized bulb at either end, with the outer field already visibly grainy/textured rather than a flat tube; a 0.2s/0.5s/1s dwell comparison at three separate spots shows clean progressive growth — small, medium, then a full bullseye with a distinct hot core, visible ring, and soft bloom by 1s; a long straight line, a curve, and a reversal/backtrack all render as one smooth continuous core with a dusty-edged atmosphere, no scalloping, no disconnection at the reversal's return point; one Fill sweep versus three overlapping sweeps shows the three-pass box visibly denser and more atmospherically built-up at the edges, confirming layered buildup survives the outer-field rewrite. No console errors from the rendering path (the only errors observed were the same pre-existing, unrelated MediaPipe hand-tracking model-fetch failures as every prior pass). Cap sizes, manufacturer anchors, and the dual-plume architecture itself were not touched.

**Files:** `SprayBrushEngine.ts` (`resolvePinkDotOuterFieldShells`, `resolveOuterFieldTexture`, `resolvePinkDotDwellScale`, `PLUME_DWELL_FULL_MS` added; `renderPinkDotOuterBand` replaced by `renderPinkDotOuterField`; `renderPinkDotInnerCore`'s boolean `hasPrevious` parameter replaced by a continuous `endpointScale`; new `pinkDotDwellMs` engine state, reset in `beginStroke()`; `renderSegment`'s plume branch now computes the dwell gate and threads `dwellScale` through), `SprayCapPresets.ts` (Pink Dot Fat's preset comment and the shared `plume*` field JSDoc updated for the outer-field/dwell mechanism — no field values or `SprayCapPreset` shape changes), `SprayBrushEngine.test.ts` (15 new tests: outer-field annular bias and continuity, texture determinism/non-zero-floor, dwell-scale floor/ramp/cap, and three rendered-behavior tests proving a bare click stays small, dwell strength never leaks into the next moving segment, and a mid-stroke pause rebuilds from a clean floor; `lineWidthRecordingContext` gained the transform/gradient no-ops the outer field's ellipse path needs). 434/434 tests passing, `tsc --noEmit` clean, production build clean.

## Pink Dot Correction — Explicit Core/Gap/Outer-Ring (a TRUE Radial Density Minimum)

Human visual review of the outer-field refinement above found it still materially wrong: the radial profile read as "dense core → fuzzy edge → continuous outer haze," not the target "dense inner core → visibly lower-density gap/moat → distinct outer ring → fading mist." The previous pass's concentric solid-disk "shells" could never produce a genuine gap — a disk of radius R always covers everything from 0 to R, so stacking several can only ever raise density in that region, never dip it.

**Radial profile, fixed — a real mathematical minimum.** `resolvePinkDotOuterFieldStops` replaces the shell/texture mechanism with a genuine four-zone `createRadialGradient` stop list (offsets as fractions of `outer.mistRadius`): core-edge (0–0.35, a moderate value below the ring peak) → a strictly suppressed moat minimum at 0.55 (`MOAT_SUPPRESSION_RATIO` — small enough that `density(moat) < density(core-edge)` and `density(moat) < density(ring)` hold structurally for any tuned ring opacity, and small enough that the moat's per-draw alpha stays low even after a full second of continuous dwell compositing, which would otherwise saturate it toward full opacity and erase the gap the same way blur would) → a ring peak at 0.8 → a mist tail fading to zero at 1.0. A radial gradient is evaluated natively per-pixel by the canvas, so the moat is a real dip in the density function itself — used for a true stationary dwell, stamped as one circle (an ellipse under flare, the whole gradient squashed as one unit so the moat/ring spacing survives elongation intact).

**The first fix for moving strokes was itself wrong, and was replaced.** Sweeping that same gradient repeatedly along a path (one stamp per sample point, densely spaced) reproduced the exact bug it was meant to fix: the union of many overlapping full disks fills the moat right back in, because a point sitting in one stamp's moat falls inside a neighboring stamp's ring. Live pixel sampling of a slow moving line confirmed a completely flat profile with no dip anywhere. Fixed properly with `resolvePinkDotOuterFieldBands`: a moving segment instead sweeps the ring and mist zones as two OFFSET PARALLEL strokes at a fixed perpendicular distance from the path — an offset stroke's own shape is exactly "constant perpendicular distance from the path," which is what a swept ring/mist band actually is, and the moat itself is simply left unpainted between the core and the ring (reduced deposition, not a "hole" punched through whatever else is on the canvas via a destructive compositing trick, which was considered and rejected as unsafe on a shared persistent paint layer).

**Overspray also had to respect the moat.** Even with the structural bands fixed, live pixel sampling still showed a flat profile — `renderOverspray`'s speckle particles (spread ~50 world units, biased toward the center) were statistically refilling the gap. A first fix (a simple minimum-radius floor around each sample point) failed too: a particle escaping its own local exclusion circle ALONG the travel direction can still land back on the centerline via a neighboring segment's own sampling. Fixed by decomposing the particle offset into along-path and perpendicular components (the LOCAL pre-rotation frame `renderOverspray` already computes) and flooring only the perpendicular one — `minSpreadRadius`, 0 (an exact no-op) for every cap but Pink Dot, whose call site passes the moat's own outer radius.

**Which technique fires needed its own fix.** Gating the stamp-vs-rails choice on raw per-segment distance alone (reusing the existing dwell-size gate) caused a THIRD flat-profile failure: the app's real curve-smoothing pipeline subdivides even genuine, deliberate slow movement into many fine sub-segments (observed distances ~1.5–3 world units per segment) that individually fall below the distance threshold without the pointer having paused at all — misclassifying an entire slow stroke as one continuous dwell and stamping circles the whole way along it. Fixed by requiring BOTH a real accumulated dwell time (`PLUME_DWELL_STAMP_MIN_MS`, 30ms) AND `point.velocity` at or below the existing stationary threshold (`PLUME_VELOCITY_FLARE_THRESHOLD`) before switching to the stamp — velocity is the pipeline's own smoothed speed estimate and is immune to this per-segment sampling noise, unlike raw distance. The stroke's bare first point is exempt from both checks (always a dot).

**Live-verified**, confirmed with direct pixel sampling (not just visual screenshot inspection, since the gap reads as only a few percent contrast against anti-aliased/compressed screenshots): a stationary dwell dot at default size shows a clear four-zone bullseye (verified both visually at 400% wall-scale zoom and via pixel trace); a slow moving line's perpendicular cross-section now shows the density genuinely dropping to background level in the moat band on both sides of the core, rising again at the ring, before fading through the mist — confirmed both via a controlled unit-level draw-call histogram and via live canvas pixel sampling; a 0°/45° flare comparison shows the whole four-zone structure narrowing together, moat still visible; a continuous cursive-style squiggle (curves, tight loops, direction reversals) shows the core line staying connected with no interruption throughout, with the outer bands from adjacent path segments overlapping smoothly in tight curves rather than breaking; three overlapping back-and-forth Fill passes still build denser atmospheric coverage without flattening into a bucket fill. No console errors from the rendering path.

**Files:** `SprayBrushEngine.ts` (`resolvePinkDotOuterFieldStops`, `RADIAL_CORE_END_T`/`RADIAL_MOAT_END_T`/`RADIAL_RING_END_T`/`CORE_EDGE_DENSITY_RATIO`/`MOAT_SUPPRESSION_RATIO` replacing the shell/texture mechanism; `resolvePinkDotOuterFieldBands` added for the moving-segment offset-rail technique; `renderPinkDotOuterField` rewritten to branch on a new `useDwellStamp` gate; `drawPinkDotOuterFieldStamp`/`drawPinkDotOuterFieldRail` as the two draw primitives; `renderOverspray` gained an optional `minSpreadRadius` parameter with along/perpendicular decomposition; `renderSegment`'s Pink Dot branch now computes `useDwellStamp` from both dwell time and velocity, and passes an overspray moat-exclusion radius; `PLUME_DWELL_STAMP_MIN_MS` added), `SprayCapPresets.ts` (Pink Dot Fat's preset comment and the shared `plume*` field JSDoc rewritten for the true-radial-minimum/offset-rail mechanism — no field values or `SprayCapPreset` shape changes), `SprayBrushEngine.test.ts` (shell/texture tests replaced with true-radial-minimum invariant tests on `resolvePinkDotOuterFieldStops`, moving-segment gap tests using `resolvePinkDotOuterFieldBands`, an overspray-moat-exclusion test, and a regression test proving a slow continuous stroke never repeatedly stamps; dwell-endpoint tests reworked around a combined gradient/rail size proxy since the outer field's rendering mechanism now differs by dwell state; `segmentRecordingContext` extended with full stroke-geometry and arc/fill capture). 439/439 tests passing, `tsc --noEmit` clean, production build clean.
