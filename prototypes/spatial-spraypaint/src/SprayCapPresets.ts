...
export interface SprayCapPreset {
  ...
  /**
   * `plumeStochasticStationary` — whether a TRUE stationary dwell (no
   * travel direction) renders through the new organic aerosol deposition
   * field (see `SprayBrushEngine.resolvePinkDotStationaryProfile` /
   * `renderPinkDotStochasticOuterField`) instead of the plain swept-rail
   * arc-stroke every `"plume"` cap used before. A MOVING segment is
   * completely unaffected either way — this only ever changes what a
   * genuinely stationary point looks like. False on every cap, including
   * `"plume"` caps that predate this field (e.g. Track Marks, which
   * deliberately keeps the exact prior stationary look).
   */
  plumeStochasticStationary: boolean;
}
...
export const SPRAY_CAP_PRESETS: readonly SprayCapPreset[] = [
  ...
  { id: "pink-dot-fat", name: "Pink Dot Fat", family: "fat", baseRadius: 42, coreDensity: 1.46, coreOpacity: 0.34, edgeFalloff: 0.76, particleCount: 26, particleSpread: 1.2, particleSize: 0.72, particleOpacity: 0.29, flowRate: 1.48, accumulationRate: 1.38, velocityResponse: 0.42, jitter: 0.06, endpointBehavior: "punchy", splatterProbability: 0.14, dripTendency: 0.72, anisotropy: 1, haloRadius: 0, haloOpacity: 0, wiggleAmplitude: 0, wiggleFrequency: 0, depositionShape: "plume", defaultFillMode: true, ringRadius: 0, ringThickness: 0, ringOpacity: 0, centerOpacity: 0, streakLanes: 0, haloDistanceGain: 0, haloFlareAnisotropy: 0, haloDabSpacing: 0, haloRingBias: 0, plumeRingRadius: 1.7, plumeRingThickness: 0.55, plumeRingOpacity: 0.24, plumeMistRadius: 2.6, plumeMistOpacity: 0.07, plumeDistanceGain: 0.85, plumeFlareStrength: 0.55, plumeStochasticStationary: true },
  // Track Marks — TEMPORARY preservation cap. Byte-for-byte the same
  // numeric personality as Pink Dot Fat above (same id would collide, so
  // this is its own SprayCapId), kept only so the PREVIOUS turn's
  // swept-offset-rail stationary look (a clean arc-stroke ring/donut) isn't
  // lost while Pink Dot Fat's OWN stationary rendering moves to the new
  // organic aerosol deposition field. `plumeStochasticStationary: false` is
  // the only difference — Track Marks' moving-stroke rendering is identical
  // to Pink Dot Fat's (both share the untouched swept-rail code), only a
  // true stationary dwell differs between the two caps.
  { id: "track-marks", name: "Track Marks", family: "fat", baseRadius: 42, coreDensity: 1.46, coreOpacity: 0.34, edgeFalloff: 0.76, particleCount: 26, particleSpread: 1.2, particleSize: 0.72, particleOpacity: 0.29, flowRate: 1.48, accumulationRate: 1.38, velocityResponse: 0.42, jitter: 0.06, endpointBehavior: "punchy", splatterProbability: 0.14, dripTendency: 0.72, anisotropy: 1, haloRadius: 0, haloOpacity: 0, wiggleAmplitude: 0, wiggleFrequency: 0, depositionShape: "plume", defaultFillMode: true, ringRadius: 0, ringThickness: 0, ringOpacity: 0, centerOpacity: 0, streakLanes: 0, haloDistanceGain: 0, haloFlareAnisotropy: 0, haloDabSpacing: 0, haloRingBias: 0, plumeRingRadius: 1.7, plumeRingThickness: 0.55, plumeRingOpacity: 0.24, plumeMistRadius: 2.6, plumeMistOpacity: 0.07, plumeDistanceGain: 0.85, plumeFlareStrength: 0.55, plumeStochasticStationary: false },
