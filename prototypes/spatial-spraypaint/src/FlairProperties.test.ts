...
describe("FlairOverrideStore -- MODE DEFAULT -> SESSION MODIFICATION -> EFFECTIVE VALUE", () => {
  it("starts empty, defaulting every (cap, mode) pair to that mode's own canonical values", () => {
    ...
    expect(getFlairOverride(EMPTY_FLAIR_OVERRIDES, "pink-dot-fat", "wall")).toEqual({});
  });
  ...
  it("resolveEffectiveFlairParams merges an override on top of the mode's own canonical defaults, including the cap-relative size envelope", () => {
    const store = setFlairOverride(EMPTY_FLAIR_OVERRIDES, "pink-dot-fat", "wall", { flairAmount: 2 });
    const effective = resolveEffectiveFlairParams("wall", "pink-dot-fat", PINK_DOT_BASE_RADIUS, getFlairOverride(store, "pink-dot-fat", "wall"));
    const defaults = getFlairProControlMetadata("wall");
    const sizeDefaults = getFlairSizeDefaults("wall", PINK_DOT_BASE_RADIUS, "fat");
  ...
  it("size envelope defaults are cap-relative -- a smaller cap baseRadius yields a smaller envelope, same tier", () => {
    const pinkDotEffective = resolveEffectiveFlairParams("wall", "pink-dot-fat", 42, {});
    const needleLikeEffective = resolveEffectiveFlairParams("wall", "pink-dot-fat", 5, {});
    expect(needleLikeEffective.flairMinSize).toBeLessThan(pinkDotEffective.flairMinSize);
    expect(needleLikeEffective.flairMaxSize).toBeLessThan(pinkDotEffective.flairMaxSize);
  });

  it("cap TIER changes the envelope even at the SAME baseRadius -- fat (Pink Dot Fat / New York Fat) widens further than mid (any other cap id, default tier)", () => {
    const fatEffective = resolveEffectiveFlairParams("wall", "pink-dot-fat", 42, {});
  ...
  it("an inverted Min/Max override (Max below Min) never collapses to a zero-or-negative span", () => {
    const store = setFlairOverride(EMPTY_FLAIR_OVERRIDES, "pink-dot-fat", "wall", { flairMinSize: 20, flairMaxSize: 10 });
    const effective = resolveEffectiveFlairParams("wall", "pink-dot-fat", PINK_DOT_BASE_RADIUS, getFlairOverride(store, "pink-dot-fat", "wall"));
    expect(effective.flairMaxSize).toBeGreaterThan(effective.flairMinSize);
  });

  it("per-brush session modifications do not leak -- setting Pink Dot Fat never affects New York Fat's store entry", () => {
    let store = setFlairOverride(EMPTY_FLAIR_OVERRIDES, "pink-dot-fat", "wall", { flairAmount: 2 });
    store = setFlairOverride(store, "new-york-fat", "wall", { flairAmount: 0.3 });
    expect(getFlairOverride(store, "pink-dot-fat", "wall")).toEqual({ flairAmount: 2 });
    expect(getFlairOverride(store, "new-york-fat", "wall")).toEqual({ flairAmount: 0.3 });
  });
  ...
  it("per-mode session modifications do not leak -- changing Wall parameters never mutates Blackbook's defaults for the same brush", () => {
    const store = setFlairOverride(EMPTY_FLAIR_OVERRIDES, "pink-dot-fat", "wall", { flairAmount: 2, flairMaxSize: 90 });
    expect(getFlairOverride(store, "pink-dot-fat", "blackbook")).toEqual({});
    const blackbookEffective = resolveEffectiveFlairParams("blackbook", "pink-dot-fat", PINK_DOT_BASE_RADIUS, getFlairOverride(store, "pink-dot-fat", "blackbook"));
    const blackbookDefaultEffective = resolveEffectiveFlairParams("blackbook", "pink-dot-fat", PINK_DOT_BASE_RADIUS, {});
  ...
  it("switching mode back preserves the earlier mode's own session modifications, coherently", () => {
    let store = setFlairOverride(EMPTY_FLAIR_OVERRIDES, "pink-dot-fat", "wall", { flairAmount: 1.8 });
    store = setFlairOverride(store, "pink-dot-fat", "blackbook", { bloomResponse: 0.1 });
    // Switching to blackbook and back to wall (no writes on wall in between) must still show wall's own earlier tweak.
    expect(getFlairOverride(store, "pink-dot-fat", "wall")).toEqual({ flairAmount: 1.8 });
    expect(getFlairOverride(store, "pink-dot-fat", "blackbook")).toEqual({ bloomResponse: 0.1 });
  });

  it("Reset Property clears one property back to the mode default, leaving sibling properties and other modes untouched", () => {
    let store = setFlairOverride(EMPTY_FLAIR_OVERRIDES, "pink-dot-fat", "wild", { flairAmount: 2, bloomResponse: 0.5 });
    store = resetFlairProperty(store, "pink-dot-fat", "wild", "flairAmount");
    expect(getFlairOverride(store, "pink-dot-fat", "wild")).toEqual({ bloomResponse: 0.5 });
  });

  it("Reset Flair clears the WHOLE mode bundle at once, leaving other modes for the same brush untouched", () => {
    let store = setFlairOverride(EMPTY_FLAIR_OVERRIDES, "pink-dot-fat", "wall", { flairAmount: 2, flairMaxSize: 90 });
    store = setFlairOverride(store, "pink-dot-fat", "blackbook", { flairSmoothing: 0.9 });
    store = resetFlairMode(store, "pink-dot-fat", "wall");
    expect(getFlairOverride(store, "pink-dot-fat", "wall")).toEqual({});
    expect(getFlairOverride(store, "pink-dot-fat", "blackbook")).toEqual({ flairSmoothing: 0.9 });
  });

  it("drops an entirely-empty brush entry from the store once its last mode is reset, without touching other brushes", () => {
    let store = setFlairOverride(EMPTY_FLAIR_OVERRIDES, "pink-dot-fat", "wall", { flairAmount: 2 });
    store = setFlairOverride(store, "new-york-fat", "wall", { flairAmount: 0.5 });
    store = resetFlairMode(store, "pink-dot-fat", "wall");
    expect(store["pink-dot-fat"]).toBeUndefined();
    expect(getFlairOverride(store, "new-york-fat", "wall")).toEqual({ flairAmount: 0.5 });
  });
...
describe("getFlairPropertyRows -- pure row descriptors for Brush Studio's FLAIR group", () => {
  it("returns exactly the six numeric controls (Min/Max Size replacing the removed Range), each carrying its effective value and modified flag", () => {
    const effective = resolveEffectiveFlairParams("wall", "pink-dot-fat", PINK_DOT_BASE_RADIUS, { flairAmount: 2 });
  ...
  it("every row's value stays within its own declared min/max bounds for every mode's canonical defaults", () => {
    for (const mode of ["wall", "blackbook", "wild"] as const) {
      const effective = resolveEffectiveFlairParams(mode, "pink-dot-fat", PINK_DOT_BASE_RADIUS, {});
