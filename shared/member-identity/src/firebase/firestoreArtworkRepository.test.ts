...
describe("legacy Artwork compatibility", () => {
  ...
  it("hydrates a persisted pageFrame (Blackbook Spatial Workspace V1)", () => {
    const time = Timestamp.fromDate(new Date("2026-01-01T00:00:00Z"));
    const artwork = decodeArtworkData("blackbook-page-1", {
      creatorId: "member-1", createdAt: time, updatedAt: time,
      surfaceId: "blackbook:studio-rich-main:page:page-1",
      pageFrame: { x: 0, y: 0, width: 1, height: 1 },
      composition: { bounds: { minX: 0.1, minY: 0.2, maxX: 0.3, maxY: 0.4 }, startedAt: time, lastEditedAt: time },
      marks: [{ id: "mark-1", type: "stroke", createdAt: time, geometry: { format: "local-2d-stroke-v1", points: [{ x: 0.1, y: 0.2 }, { x: 0.3, y: 0.4 }] }, style: { color: "#171412", width: 7, opacity: 0.9 } }],
      state: "draft", visibility: "private",
    });
    expect(artwork.pageFrame).toEqual({ x: 0, y: 0, width: 1, height: 1 });
  });

  it("omits pageFrame for a document that never had one (Map/Blank, or legacy Blackbook)", () => {
    const time = Timestamp.fromDate(new Date("2026-01-01T00:00:00Z"));
    const artwork = decodeArtworkData("blackbook-no-frame", {
      creatorId: "member-1", createdAt: time, updatedAt: time,
      surfaceId: "blackbook:studio-rich-main:page:page-1",
      composition: { bounds: { minX: 0.1, minY: 0.2, maxX: 0.3, maxY: 0.4 }, startedAt: time, lastEditedAt: time },
      marks: [{ id: "mark-1", type: "stroke", createdAt: time, geometry: { format: "local-2d-stroke-v1", points: [{ x: 0.1, y: 0.2 }, { x: 0.3, y: 0.4 }] }, style: { color: "#171412", width: 7, opacity: 0.9 } }],
    ...
    expect(artwork).not.toHaveProperty("pageFrame");
  });
