...
const NOTE_COLORS = {
  "C":"#ff2a22", "C#":"#ff6d39", "D":"#ff9b1f", "D#":"#ffbf2f",
  "E":"#e5d645", "F":"#b9d957",  "F#":"#35b96d", "G":"#28bea0",
  "G#":"#2bb8d2","A":"#498cff",  "A#":"#7a62ff", "B":"#f05abf",
};
...
SBE.DrawerSystem.registerDrawer({
  ...
  initialize: async function() { /* load saved library from localStorage */ },
  mount:      function(container) { /* render editor UI into container */    },
  unmount:    function(container) { /* preserve state, clear container */    },
  destroy:    function()          { /* flush library save, release canvas */ },
});
```
...
WOS.GlyphRenderer = {
  ...
  renderToBitmap: function(entry, size) { /* returns Promise<ImageBitmap> */ },
};
```
  ...
  <div class="drawer-view__body">
    ...
    <div class="glyph-editor-row">
      ...
      <div class="glyph-controls">
        ...
        <label>Color</label>
        <select id="glyph-color-mode">...</select>
        <label>Size</label>
...
window._wos.shortcuts = {
  suspend: function() { /* set _shortcutsSuspended = true */ },
  resume:  function() { /* set _shortcutsSuspended = false */ },
};
```
...
function exportGlyphPng(entry, size) {
  ...
  WOS.GlyphRenderer.renderGlyph(canvas.getContext("2d"), entry.note, 0, 0, size, {
