// ── MapsPaletteAuthority Tests v1.0.0 ─────────────────────────────────────────
...
//
// Run via: _wos.debug.mapsPalette.runTests()
//
// Placement: wall/systems/presentation/mapsPaletteAuthority.tests.js
// Load: AFTER mapsPaletteAuthority.js. Not required for production operation —
// safe to omit from a minimal production bundle if one is ever introduced.
// ──────────────────────────────────────────────────────────────────────────────
(function (global) {
  'use strict';
  ...
  // the surface MapsPaletteRegistry/MapsPaletteApplyAdapters use:
  // getStyle()/getLayer()/setPaintProperty()/getPaintProperty()/getZoom().
  ...
  function run(map) {
    var authority = SBE.MapsPaletteAuthority;
    var results = [];
    ...
    // ── Default coverage ───────────────────────────────────────────────────
    var def = authority.getPalette(authority.DEFAULT_PALETTE_ID);
    ...
    var duplicated = authority.duplicatePalette(created.id);
    ...
    authority.activatePalette(created.id);
    results.push(_assert('activatePalette() updates the active id', authority.getActiveId() === created.id));

    authority.activatePalette(savedActive); // restore before preview checks
    results.push(_assert('activating a prior palette deactivates the previous one',
    ...
    authority.previewPalette(duplicated.id);
    results.push(_assert('previewPalette() does not change the active id', authority.getActiveId() === savedActive));
    results.push(_assert('previewPalette() sets the preview id', authority.getPreviewId() === duplicated.id));
    ...
    results.push(_assert('endPreview() restores the active palette', authority.getActiveId() === savedActive));
    ...
    if (ids.length) {
      // Deliberately omit the first wired id to simulate a stale/older palette.
    ...
    console.log('[MapsPaletteAuthorityTests] ' + (summary.ok ? 'PASS' : 'FAIL') +
      ' — ' + (results.length - failed.length) + '/' + results.length + ' assertions passed');
    if (failed.length) console.warn('[MapsPaletteAuthorityTests] failures:', failed);

    return summary;
  }

  SBE.MapsPaletteAuthorityTests = { run: run, createMockMap: createMockMap };
