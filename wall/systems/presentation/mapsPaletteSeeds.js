// ── MapsPaletteSeeds v1.0.0 ───────────────────────────────────────────────────
// 0729C_MAPS_Episode2_Durable_Seed
// Status: active | Classification: presentation-authority / palette-seed-data
//
// Durable seed data for the "Episode 2" palette — a rule-based, group/label-
// pattern-aware color assignment (not a hardcoded per-id map), so it produces
// correct, complete coverage against whatever the CURRENT live registry
// contains, regardless of exact layer ids or registry size. This is DATA, not
// UI code — no palette colors live in palettesGallery.js/paletteDetail.js.
//
// Consumed once, by MapsPaletteAuthority.init(), only when a fresh profile has
// no saved palettes beyond Default. This makes Episode 2 durable/reproducible
// for any browser profile — it is no longer dependent on manual console
// authoring in one tester's session.
//
// Placement: wall/systems/presentation/mapsPaletteSeeds.js
// Load: AFTER mapsPaletteApplyAdapters.js, BEFORE mapsPaletteAuthority.js.
// ──────────────────────────────────────────────────────────────────────────────
(function (global) {
  'use strict';
  ...
  SBE.MapsPaletteSeeds = Object.freeze({
    buildEpisode2Seed: buildEpisode2Seed,
  });

  console.log('[MapsPaletteSeeds] loaded');
