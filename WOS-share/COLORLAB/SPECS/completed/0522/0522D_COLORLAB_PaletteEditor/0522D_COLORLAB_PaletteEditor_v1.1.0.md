...
Status: Build-Ready Lineage-Safe Editing Infrastructure
...
# Candidate Identity Doctrine

CRITICAL:
# candidate identity must remain stable across all downstream systems

Canonical candidate references must NOT use:
- hex values
- RGB strings
- display representations

Candidate identity must resolve through:
# canonical extraction candidate identity
...
```txt
sc_0001:candidate_sha256_abc123
```

All editor lineage references must use:
# stable candidate identity references

NOT derived display values.
...
Authored LAB values must be computed:
# at edit time

using the canonical conversion pipeline,
then stored as archival interpretation truth.
...
```json
{
  ...
  "provenance": {
    ...
    "editorVersion": "1.1.0",
    "createdFromRevisionId": "rev_0002"
  },
...
```json
{
  ...
  "editSummary": {
    "note": "non-authoritative human-readable annotation"
  }
}
```
...
must remain:
# editor-session infrastructure

NOT governed archival payload structure.
...
revisionId is unique and non-recycled
committedAt timestamp is present and valid
...
# Archive Transition Doctrine

The editor may:
# request archival transition

The editor does NOT:
# authorize archival eligibility

Archival eligibility requirements belong to:
# future archival governance infrastructure

The editor may not independently determine:
- archival worthiness
- archival completeness
- historical significance

without external governance authority.
...
Colorlab gains:
# build-ready lineage-safe palette editing infrastructure
