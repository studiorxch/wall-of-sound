...
Status: Build-Ready Reproducibility Infrastructure
...
Governance v1.4.0 is considered:
...
CRITICAL:
# partially transparent pixels are excluded from extraction sampling
...
# GIF Extraction Doctrine

Animated GIF extraction is:
# first-frame locked

Canonical extraction behavior:

```txt
frameIndex = 0
```

Frame selection is currently:
# engine-version-locked behavior

NOT user-controlled extraction behavior.
...
The following fields constitute:
# extraction_settings
...
All extraction_settings fields participate in:
...
CRITICAL:
# regeneration does NOT override archival history
...
If regeneration inputs are identical to an existing:
# SOURCE_CANDIDATES

record,
then:
# deduplication doctrine applies

and the existing payload is reused.

If extraction inputs differ,
regeneration creates:
# a new SOURCE_CANDIDATES record
