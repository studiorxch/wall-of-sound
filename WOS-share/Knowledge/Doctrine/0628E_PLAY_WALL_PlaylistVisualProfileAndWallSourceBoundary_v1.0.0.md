---  
location: Doctrine  
title: PLAY Playlist Visual Profile + WALL Source Boundary  
date: 2026-06-28  
...
```
## MAPS as Default WALLFor the current WOS architecture, `MAPS` is the primary/default WALL.This should remain true for a long time.`WALL_MAP` is the adopted default world surface for WOS and currently owns the main visual runtime:```textMapboxcity/world viewroutesFlightDriveWalkOrbitalplaylist visual coverage
```
...
```
Graffiti WallSubway WallKinetic Fish WallCanvas Wallpaper WallSmart Grid WallPlaylist Card Wall
```
...
```
MAPS is the default WOS world.Other WALLS may exist later as independent OBS/browser sources.Do not prematurely nest future WALLS inside MAPS.Do not prematurely split Orbital out of MAPS.
```