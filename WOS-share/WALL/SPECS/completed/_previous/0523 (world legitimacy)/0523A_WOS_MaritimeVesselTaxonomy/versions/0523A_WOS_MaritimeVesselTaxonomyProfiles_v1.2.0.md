...
date: 2026-05-23
doc_id: "0523A_WOS_MaritimeVesselTaxonomyProfiles_v1.2.0"
version: "1.2.0"
...
summary: "Defines the concrete implementation profile layer for maritime vessel taxonomy, including profile schemas, normalized scalar semantics, enum contracts, AIS mapping, UNKNOWN fallback behavior, and numerical compilation rules."
...
0523A_v1.2.0 = taxonomy implementation profile layer
```
...
The runtime may multiply an existing canonical coast window by this class-level scalar.

It may NOT introduce class-specific confidence formulas.
...
```ts
export const TAXONOMY_PROFILE_VERSION = "1.2.0";
...
export function resolveVesselClassFromAIS(
  ...
  if (aisTypeCode >= 40 && aisTypeCode <= 59) return "SERVICE";
...
1. resolve raw AIS data into `VesselClass`
2. attach the compiled taxonomy profile
3. store both in `VesselRegistry`
4. expose read-only profile state to downstream observers
